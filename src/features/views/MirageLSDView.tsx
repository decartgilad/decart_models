...
export default function MirageLSDView({ model }: ModelViewProps) {
  // Use existing hooks
  ...
  const handlePromptSubmit = React.useCallback(async (prompt: string, file: File) => {
    ...
    try {
      ...
      // Create job with real uploaded file and MirageLSD-specific parameters
      const mirageLSDInput = {
        ...uploadResult,
        originalName: file.name, // Add original filename for format validation
        generationsCount: 1, // Default to 1 generation
      }
      
      jobCreation.createJobWithModel(mirageLSDInput, prompt)
  ...
  const getStageState = React.useCallback((): 'idle' | 'loading' | 'error' | 'result' => {
    ...
    if (jobStatus.data?.status === 'succeeded' && jobStatus.data?.output) {
      ...
      const hasVideoUrl = (output.video && output.video.url) ||
                          ...
                          output.result_url // MirageLSD specific field
    ...
    <div className="space-y-xl">
      ...
      <Stage
        ...
        videoUrl={(() => {
          if (jobStatus.data?.status === 'succeeded' && jobStatus.data?.output) {
            const output = jobStatus.data.output
            // Return the first available video URL - check nested video object first, then MirageLSD specific fields
            return (output.video && output.video.url) || 
                   output.url || output.video_url || output.videoUrl || 
                   output.result_url || undefined // MirageLSD specific field
          }
          return undefined
        })()}
